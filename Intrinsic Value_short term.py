...
def intrinsic_value_calculator(ticker, terminal_growth=0.01, years=2, max_growth_cap=0.20):
    ...
    if ("Financial Services" in sector) or ("Insurance" in sector) or ("Reinsurance" in industry):
        ...
        try:
            if net_income and shares_outstanding and float(shares_outstanding) > 1000000:#makes sure the shares outstanding isn't some bs number because yfinance might return 1.2 billion as 1.2 or something like that
        ...
        terminal_pe = float(np.clip(justified_pe, 5, 14))#Use 14x as cap because historically, most matured firms that's in the financials/insurance/reinsurance has stablises at a PE of around 10-13 but I use 14 just to be conservative
        ...
        if price and book_value_per_share > 0 and price > 0:
            ...
            if ratio > 50:  # only trigger on extreme mismatch
        ...
        # sustainable ROE & future book value
        ...
        # modest weight(15%) + mild cap vs intrinsic to prevent swamping
        if intrinsic_per_share > 0:# in case it somehow returns negative, mb because net income was negative or discount rate > growth rate, which is highly improbable but just in case
            book_component = 0.15 * book_value_future
        else:
            book_component = 0
        book_component = min(book_component, intrinsic_per_share * 5)
